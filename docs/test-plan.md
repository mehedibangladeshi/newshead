...
- [ ] App launches without crashing; date header shows today's date.
- [ ] The bottom pill bar renders one pill per category from the *fetched*
      JSON, not a hardcoded list — confirms `home_screen.dart` stays
      data-driven as the taxonomy changes.
...
- [ ] A category with articles shows: source pill, headline, snippet, "Read
      more", a correctly-aspect-ratioed sharp image, and a blurred backdrop
